...
void parseArg( int argc, char **argv, param &PARAM )
{
  ...
  while ( true )
  {
    ...
    case 0:
      ...
      else if ( (string)long_options[option_index].name == "help" )
      {
        help();
        exit( EXIT_SUCCESS );
      }
  ...
  if ( !PARAM.TEMP_FIRST && !PARAM.SPAT_FIRST )
  {
    cerr << "ERROR - You MUST specify if mining is temporal first or spatial first"
         << "by using option --tempFirst XOR --spatFirst." << endl;
    help();
    exit( EXIT_FAILURE );
  }
  else if ( PARAM.TEMP_FIRST && PARAM.SPAT_FIRST )
  {
    cerr << "ERROR - You MUST only specify on type of mining, either temporal first XOR spatial first"
         << "by using option --tempFirst XOR --spatFirst." << endl;
    help();
    exit( EXIT_FAILURE );
  }
